...
const SOL_TO_USD = 102.45 // Example rate: 1 SOL = $102.45 USD
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const totalRemoved = userTransactions.filter((tx) => tx.type === "removal").reduce((sum, tx) => sum + tx.amount, 0)

    // Total liquidity added (from coin creations)
    const totalAdded = userTransactions.filter((tx) => tx.type === "creation").reduce((sum, tx) => sum + tx.amount, 0)
  ...
  const generateRandomPNL = () => {
    ...
    const updatedCoins = allCoins.map((coin: Coin) => {
      if (coin.userId === user.id) {
        // Generate a random multiplier that's at least 1.5x
        const multiplier = 1.5 + Math.random() * 2.5 // Between 1.5x and 4x

        // Calculate new profit based on investment
        const newProfit = coin.investment * (multiplier - 1)
                  ...
                  {transactions.map((tx) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium">{tx.coinName}</p>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-muted-foreground">User ID: {tx.userId.substring(0, 8)}...</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <CardHeader className="pb-2">
          <div className="flex justify-between items-start">
            <div className="flex items-center">
              <div className="w-10 h-10 rounded-full overflow-hidden mr-3 bg-secondary/50">
                <img src={coin.image || "/placeholder.svg"} alt={coin.name} className="w-full h-full object-cover" />
              </div>
              <div>
          ...
          <div className="flex justify-between">
            <Button
              ...
              disabled={
                ...
                Number.parseFloat(liquidityAmount) > currentLiquidity + 0.001 // Add small tolerance for floating-point precision
              }
            >
              {isRemoving ? (
                <>
  ...
  return (
    <Card className="overflow-hidden border-border">
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          <div className="flex items-center">
            <div className="w-10 h-10 rounded-full mr-3 bg-secondary animate-pulse"></div>
            <div>
      ...
      <CardContent>
        <div className="pt-3 border-t border-border">
          <div className="flex justify-between items-center">
            ...
            <div className="text-right">
              <div className="h-3 w-20 bg-secondary animate-pulse rounded"></div>
              <div className="h-5 w-16bg-secondary animate-pulse rounded mt-1"></div>
            </div>
          </div>
        </div>
      </CardContent>
